...
import { Plus, Activity, HardHat, Pencil, Trash2, MoreVertical, Droplets, Building2, Wallet, ShieldAlert } from "lucide-react";
    ...
    <main className="flex-1 px-4 py-8 md:pl-80 md:pr-12 md:pt-32 mb-24 md:mb-0 overflow-hidden">
      {isProfileLoading ? (
        <div className="flex flex-col items-center justify-center h-[60vh] gap-4">
          ...
          <p className="text-primary font-mono text-[10px] uppercase tracking-widest opacity-50">Syncing Intelligence...</p>
        </div>
      ) : (profile && profile.role !== "Admin") || activeDivision.id !== "nalakath-holdings-main" ? (
        <div className="flex flex-col items-center justify-center h-[60vh] gap-4 text-center">
          <ShieldAlert className="h-12 w-12 text-destructive opacity-50" />
          <h2 className="text-xl font-bold uppercase tracking-tight">Access Restricted</h2>
          <p className="text-muted-foreground text-sm max-w-xs">Admin level authorization required for HQ Projects.</p>
        </div>
      ) : (
          ...
          <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
            {isLoading ? (
              ...
              projects?.map((project) => (
                <Card key={project.id} className="gold-glass control-center-card overflow-hidden group hover:scale-[1.01] ios-transition relative border-primary/20">
